...
export default function ScanTriggerFailureDialog({
    ...
    return (
        <div className="absolute inset-0 z-[90] flex items-center justify-center bg-[#0F172A]/55 p-8 backdrop-blur-[1px]">
            <div role="alertdialog" aria-modal="true" aria-labelledby="scan-trigger-failure-title" aria-busy={busy} className="w-[480px] rounded-2xl border border-[#FCA5A5] bg-white p-6 shadow-2xl">
                ...
                <div className="mt-6 flex justify-end gap-3">
                    <button type="button" onClick={onReturnToConfirm} disabled={busy} className="h-10 rounded-lg border border-[#94A3B8] bg-white px-4 text-[12px] font-bold text-[#475569] hover:bg-slate-50 disabled:cursor-wait disabled:opacity-55">
                        {busy ? "处理中…" : "返回确认"}
                    </button>
                    <button type="button" onClick={onRetry} disabled={busy} className="flex h-10 items-center gap-1.5 rounded-lg bg-[#2563EB] px-4 text-[12px] font-bold text-white hover:bg-[#1D4ED8] disabled:cursor-wait disabled:opacity-55">
                        <RotateCcw size={15} className={busy ? "animate-spin" : undefined} />
                        {busy ? "处理中…" : "重新尝试"}
                    </button>
                </div>
            </div>
