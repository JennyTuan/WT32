...
from .demo_dicom_registry import (
    build_four_d_data_review_manifest,
    build_four_d_manifest,
...
app.include_router(service_state.router, prefix="/api")
...
@app.get("/api/demo-dicom/fourd-data-review")
def get_four_d_data_review_manifest():
    manifest = build_four_d_data_review_manifest()
    if manifest is None:
        raise HTTPException(status_code=404, detail="4D simulated data-review reference is unavailable")
    return manifest
