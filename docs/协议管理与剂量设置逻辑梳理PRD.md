...
- 超阈值标记：本次扫描是否超过报告剂量阈值（是 / 否）。
- 超阈值附加信息（仅在标记为"是"时填充）：命中的报告剂量阈值（CTDIvol 阈值、DLP 阈值）、超阈值判定时所采用的处理策略、二次确认人、确认时间。

业务规则：

- 剂量记录在生成时冻结上述快照字段，即使后续协议、会话、患者或报告剂量阈值被修改或删除，记录仍保持当时的语义。
- 剂量记录不反写协议。
- 剂量记录不反写系统报告剂量阈值。
- 若当前扫描剂量达到或超过命中的报告剂量阈值，则将超阈值标记置为"是"，并按系统超阈值处理策略执行。
- 若本次扫描所属"部位 × 年龄组"在阈值表中无匹配条目，则不进行判断，超阈值标记置为"否"。

### 3.7.1 超阈值标记的用途

- 剂量日志列表对标记为"是"的记录显示醒目的"超阈值"徽标，便于技师快速识别；标记为"否"的记录不展示任何额外标记。
- 剂量日志支持"仅显示超阈值"切换，方便快速汇总。
- 剂量日志页底部应展示当前筛选条件下的超阈值记录数。
- 该标记一经写入不可由用户在剂量日志页直接修改；如发现误判，需通过审计流程纠正。
...
- 超阈值处理策略来自系统剂量设置。
- 生成剂量记录并绑定扫描会话；在写入时计算并冻结超阈值标记（是 / 否）。

执行后：

- 剂量记录进入剂量日志，超阈值的记录在列表中展示醒目"超阈值"徽标。
- 剂量记录不修改系统报告剂量阈值。
- 剂量记录不修改协议。
...
18. 剂量记录在写入时冻结超阈值标记，标记为是 / 否两态。
19. 剂量日志列表对超阈值记录展示醒目徽标，支持"仅显示超阈值"筛选，并在底部汇总当前筛选条件下的超阈值记录数。
20. 已超阈值的剂量记录需保存命中的报告剂量阈值快照、处理策略，以及（若策略为强制二次确认）二次确认人和确认时间。
21. 所有用户可见页面均不出现 AEC 文案。
22. DOM 噪声等级在剂量设置、协议编辑、扫描确认页均为低、中、高三级。
